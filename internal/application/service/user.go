...
//
// When users.tenant_id is non-zero we still verify both the home tenant row
// and the active membership still exist. A missing tenant row means the
// workspace was deleted while the pointer kept targeting it; a missing
// membership means the member was removed (#2586). Either makes the home
// pointer stale, so we clear it (clearStaleHomeTenant) and re-resolve.
// Superusers that can access every tenant skip the membership gate.
func (s *userService) homeOrFirstMembershipTenant(ctx context.Context, user *types.User) uint64 {
	...
	// The home workspace itself may have been deleted while users.tenant_id
	// kept pointing at it (workspace-delete deadlock). Treat a missing tenant
	// row the same as no home: clear the stale pointer and re-resolve.
	if s.tenantService != nil {
		if _, err := s.tenantService.GetTenantByID(ctx, user.TenantID); err != nil {
			logger.Warnf(ctx,
				"homeOrFirstMembershipTenant: home tenant %d missing for user %s, clearing stale home and re-resolving: %v",
